# 90-Day FDE Acceleration Plan

## Month 1 — Core motion
Primary objective: build one complete FDE-style capstone.

Outcomes:
- one audited workflow
- one deployment-grade system
- one eval pack
- one stakeholder-ready case study

## Month 2 — Range and repetition
Primary objective: run the same motion again in a different workflow domain.

Suggested second domains:
- support ops
- sales ops
- recruiting ops
- finance / procurement
- internal knowledge workflows

Outcomes:
- better pattern recognition
- stronger audit discipline
- improved reuse of templates and eval structures

## Month 3 — Market-facing proof
Primary objective: use the artifacts in real conversations.

Actions:
- show the case studies to operators or founders
- run a small audit sprint for a real team
- capture objections and deployment fears
- refine the offer around de-risked value

Outcomes:
- real buyer language
- stronger deployment intuition
- clearer niche positioning

## Graduation criteria
By day 90, you should be able to produce:
- a workflow audit in hours, not days
- an agent/system prototype with traces and guardrails
- a credible eval pack
- a rollout plan integrated with an existing stack
- a pitch that sounds like an operator, not just a builder
